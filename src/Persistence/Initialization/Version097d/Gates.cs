...
public class Gates : InitializerBase
{
    ...
    private void CreateWarpEntries(IDictionary<short, ExitGate> gates)
    {
        ...
        this.AddWarpIfMissing(gates, 21, "Tarkan", 8000, 140, 57);
        this.AddWarpIfMissing(gates, 23, "Icarus", 10000, 170, 63);
    }
    ...
    private void AddWarpIfMissing(IDictionary<short, ExitGate> gates, ushort index, string name, int costs, int levelRequirement, short gateNumber)
    {
        if (this.GameConfiguration.WarpList.Any(w => w.Index == index))
        {
            return;
        }

        if (!gates.TryGetValue(gateNumber, out var gate))
        {
            return;
        }

        this.GameConfiguration.WarpList.Add(this.CreateWarpInfo(index, name, costs, levelRequirement, gate));
    }
