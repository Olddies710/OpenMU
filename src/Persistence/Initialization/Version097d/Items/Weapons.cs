...
internal class Weapons : InitializerBase
{
    ...
    public override void Initialize()
    {
        ...
        this.CreateWeapon(0, 3, 0, 20, 1, 3, true, "Katana", 16, 16, 26, 35, 27, 0, 0, 80, 40, 0, 0, 0, 1, 0);
        ...
        this.CreateWeapon(0, 14, 0, 22, 1, 3, true, "Lightning Sword", 59, 59, 67, 30, 50, 0, 0, 90, 50, 0, 0, 0, 1, 1);
        ...
        this.CreateWeapon(0, 19, 0, 22, 1, 4, false, "Divine Sword of Archangel", 86, 220, 230, 45, 168, 0, 0, 140, 50, 0, 0, 0, 1, 0, 1);
        this.CreateWeapon(0, 20, 0, 22, 1, 4, true, "Knight Blade", 140, 107, 115, 35, 90, 0, 0, 116, 38, 0, 0, 0, 2, 0, 0);
        this.CreateWeapon(0, 31, 0, 56, 2, 4, true, "Rune Blade", 100, 104, 130, 35, 93, 104, 0, 135, 62, 9, 0, 0, 0, 1);
    ...
    protected void CreateAmmunition(byte @group, byte number, byte slot, byte width, byte height, bool dropsFromMonsters, string name, byte dropLevel, byte durability, int wizardClass, int knightClass, int elfClass)
    {
        ...
        var qualifiedCharacterClasses = this.GameConfiguration.DetermineCharacterClasses(wizardClass > 0, knightClass > 0, elfClass > 0);
        ...
        int wizardClass, int knightClass, int elfClass, int magicGladiatorClass = 0, bool isAmmunition = false)
    {
        ...
        item.Durability = durability;
        var classes = CharacterClasses.None;
        if (wizardClass > 0)
        {
            classes |= CharacterClasses.DarkWizard | CharacterClasses.SoulMaster | CharacterClasses.MagicGladiator;
        }

        if (knightClass > 0)
        {
            classes |= CharacterClasses.DarkKnight | CharacterClasses.BladeKnight | CharacterClasses.MagicGladiator;
        }

        if (elfClass > 0)
        {
            classes |= CharacterClasses.FairyElf | CharacterClasses.MuseElf;
        }
        ...
        var qualifiedCharacterClasses = this.GameConfiguration.DetermineCharacterClasses(classes, true);
        qualifiedCharacterClasses.ToList().ForEach(item.QualifiedCharacters.Add);
